...
# <font color='green'>Part 2-Bayes' Theorem</font>
...
A medical test gives us a good example of why Bayes' theorem is useful.

Suppose:

- 1% of women have breast cancer.
- If a woman has breast cancer, the mammogram is positive 80% of the time.
- If a woman does not have breast cancer, the mammogram is still positive 9.6% of the time.

Now suppose a woman gets a **positive mammogram**.

We know:

\[
P(\text{Positive}\mid\text{Cancer}) = 0.80
\]

But what we actually want to know is:

\[
P(\text{Cancer}\mid\text{Positive})
\]

These are not the same qquestion, we need to combine the test result with how common cancer is in the first place.
