---
hide:

  - toc
  
tags:
    - Case Studies
    - System Design
    - Lessons Learned
---

# Engineering Studies
This section contains engineering evaluations, design studies, and practical insights from software engineering projects.

---
<font color='green'> **Technical Evaluations** </font>
<br>*(Framework and technology assessments)*

<table border="1">
    <tr>
        <td><a href="openrmf_assessment.html">OpenRMF: Engineering Assessment</a></td>
    </tr>
</table>

---
<font color='green'> **Comparisons** </font>
<br>*(Technology, tools and engineering comparisons)* 

<table border="1">
    <tr>
        <td>
            <a href="../blog/2026/07/29/why-c-is-not-my-first-choice.html">
                Why C++ Is NOT My First Choice
            </a>
        </td>
        <td>
            <a href="../blog/2026/07/31/persistent-vs-non-persistent-connections-choosing-the-right-communication-model.html">
                Persistent vs Non-Persistent Connections: Choosing the Right Communication Model
            </a>
        </td>
    </tr>
    <tr>
        <td>
            <a href="../blog/2026/08/03/bare-metal-vs-operating-systems-in-embedded-systems-choosing-the-right-software-architecture.html">
                Bare Metal vs OS-Based Embedded Systems
            </a>
        </td>
        <td>
            <!-- Future article -->
        </td>
    </tr>
</table>


---
<font color='green'> **Design Studies** </font>
<br>*(Architecture and design analyses)*

---
<font color='green'> **Lessons Learned** </font>
<br>*(Implementation insights and retrospectives)*
 



