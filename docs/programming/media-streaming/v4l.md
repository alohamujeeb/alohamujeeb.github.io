...
- Tthere are utilities built on top of V4L2
...
### 1. MMAP (memory-mapped buffers) — most common
...
### 2. USERPTR (user-provided buffers)
...
### 3. READ method (simplest, least efficient)
...
## 4. Stream with output file (MMAP example)
```
v4l2-ctl -d /dev/video0 \
